  ...
  (let* ((col-name "..count..")
    ...
    (prog1 df
      (flet ((add-it (index row)
               ...
               (let* ((vals (map-aesthetics aest frame row))
                      (xval (getf vals :x)))
                 (incf (gethash xval fr 0)))))
        (map-data-frame-rows frame t #'add-it)
        (loop for key being the hash-key of fr using (hash-value value)
              collect (list key value) into rows
              finally (apply #'add-rows! df rows))
        (setf (slot-value stat 'aest) new-aest)))))
